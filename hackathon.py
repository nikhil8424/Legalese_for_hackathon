...
def legalese_ui():
    ...
    # Container frame
    container = tk.Frame(root, bg="#F9F0EE")
    container.pack(expand=True, fill="both", padx=20, pady=20)

    # Left pane
    left_pane = tk.Frame(container, bg="#F9F0EE", width=300, borderwidth=1, relief="solid", padx=20, pady=20)
    left_pane.pack(side="left", fill="y")

    # Add content to the left pane
    welcome_text = [
    ("Hey there!", 16),
    ("We know it's not perfect yet",12),
    ("and we sincerely apologize for any inconvenience caused by the current performance.", 12),
    ("But here’s the exciting part—this is just the beginning!", 12),
    ("In future versions, we’re planning to integrate cutting-edge AI and Machine Learning technologies to enhance accuracy, speed, and intelligence.", 12),
    ("A revamped, intuitive user interface is on the way to ensure a seamless user experience.", 12),
    ("We’ll also add more powerful features and improve the current functionalities based on your valuable feedback.", 12),
    ("How does it work now? Simple: Upload -> Process -> Select Function -> Get Results!", 12),
    ("Thank you for your patience and support as we work toward making LegalEase your ultimate legal assistant.", 12),
    ("Let’s take the first step together. Your hassle-free legal journey starts here!", 12)
]


    for text, size in welcome_text:
        label = tk.Label(left_pane, text=text, font=("Arial", size), bg="#ffe9e3", anchor="w", justify="left", wraplength=260)
        label.pack(anchor="w", pady=5)

    # Right pane
    right_pane = tk.Frame(container, bg="#ffe9e3", width=700, padx=20, pady=20)
    right_pane.pack(side="left", fill="both", expand=True)

    # Title in the right pane
    right_title = tk.Label(right_pane, text="Legalize Application", font=("Arial", 24), bg="#F9F0EE")
    right_title.pack(pady=10)

    # Language selection dropdown
    language_var = tk.StringVar(value="English")
    language_label = tk.Label(right_pane, text="Select Language:", font=("Arial", 14), bg="#F9F0EE")
    language_label.pack(pady=5)
    language_options = ["English", "Marathi", "Hindi", "Tamil"]
    language_menu = tk.OptionMenu(right_pane, language_var, *language_options)
    language_menu.config(font=("Arial", 14), bg="#c7b198")
    ...
    # Upload section
    upload_section = tk.Frame(right_pane, bg="#F9F0EE")
    upload_section.pack(fill="x", pady=10)

    upload_label = tk.Label(upload_section, text="Upload your File here (png, jpg, pdf)", font=("Arial", 14), bg="#F9F0EE")
    upload_label.pack()

    upload_button = tk.Button(upload_section, text="Upload", font=("Arial", 16), bg="#c7b198", fg="black", 
    ...
    identify_button = tk.Button(labels_frame, text="Identify", font=("Arial", 16), bg="#A86254", fg="white", relief="raised", 
                                padx=10, pady=10, cursor="hand2", command=identify_document)
    identify_button.pack(side="left", expand=True, fill="x", padx=5)

    keypoints_button = tk.Button(labels_frame, text="Key Points", font=("Arial", 16), bg="#A86254", fg="white", relief="raised", 
    ...
    plain_lang_btn = tk.Button(labels_frame, text="Plain Language", font=("Arial", 16), bg="#A86254", fg="white", relief="raised", 
    ...
    for btn_text in buttons:
        if btn_text == "Identify":
            ...
            btn = tk.Button(labels_frame, text=btn_text, font=("Arial", 16), bg="#A86254", fg="white", relief="raised", 
